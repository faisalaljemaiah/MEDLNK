...
export default async function FollowersPage({
  ...
  // an admin account has no public profile, so it has no public follower
  // list either.
  if (!profile || profile.is_admin) notFound();

  const people = await getFollowList(supabase, profile.id, "followers", user?.id ?? null);
    ...
    <div>
      <div className="flex items-center gap-2 px-2 pt-3">
        ...
        <h1 className="font-headline text-lg text-text">
          {t(locale, "profile.followersTitle")}
        </h1>
      </div>
      <FollowList
        people={people}
        viewerId={user?.id ?? null}
        emptyMessage={t(locale, "profile.emptyFollowersList")}
