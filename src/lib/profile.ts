...
/**
 * The Discover page's People category (IA redesign) — filtered in JS over
 * one query, same shape as everything else search-adjacent in this codebase
 * (getFeedCases + client-side filtering in search/page.tsx) — deliberately
 * not a PostgREST `.or()` filter string built from raw user input, since
 * that mini-language treats commas/dots/parens as syntax and unescaped user
 * text could manipulate which columns or rows a query actually matches.
 * Empty query returns [] rather than every profile on the platform.
 */
export async function searchProfiles(
  ...
  return data
    .filter((p): p is PersonResult => Boolean(p.handle))
    .filter((p) =>
      [p.full_name, p.handle, p.specialty, p.role]
        .filter(Boolean)
