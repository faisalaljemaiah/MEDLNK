...
export function ReelView({ cases, path }: { cases: FeedCase[]; path: string }) {
  if (cases.length === 0) {
    return (
      <div className="flex h-[calc(100dvh-129px)] w-full shrink-0 items-center justify-center bg-bg text-sm text-muted">
  ...
  return (
    <div className="no-scrollbar h-[calc(100dvh-129px)] w-full shrink-0 snap-y snap-mandatory overflow-y-scroll">
      {cases.map((c) => (
