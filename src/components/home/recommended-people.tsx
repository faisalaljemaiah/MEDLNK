...
export function RecommendedPeople({
  ...
  path: string;
  locale: Locale;
  /** The Following tab's empty state reuses this same card row under a more
   *  encouraging heading ("Follow a few people to get started") rather than
   *  the passive "People you may know" this section otherwise carries. */
  titleKey?: TranslationKey;
  /** False inline in the Following tab's empty state, where a top border
   *  would duplicate the divider that section already sits under. */
  bordered?: boolean;
}) {
  if (people.length === 0) return null;

  return (
    <section className={bordered ? "mt-5 border-t border-line pt-4" : "mt-4"}>
