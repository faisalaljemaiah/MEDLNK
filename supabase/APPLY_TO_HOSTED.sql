...
--
-- Dropped first: 0036 later in this file changes this function's return type
-- (void -> setof uuid) for push dispatch, which `create or replace` can't do,
-- so this has to be drop-then-create too or a second full paste of this file
-- errors trying to "create or replace" back to void over the setof uuid
-- version 0036 already installed.
...
--
-- Dropped first: 0036 later in this file changes the return type for push
-- dispatch, which `create or replace` can't do across a second full paste.
...
-- ============================================================================
-- 0015_safety_alerts.sql — a hazard that interrupts rather than waits
-- ============================================================================
-- Acknowledgements --------------------------------------------------------------
...
create table if not exists public.safety_alert_acks (
  case_id uuid not null references public.cases (id) on delete cascade,
  user_id uuid not null references public.profiles (id) on delete cascade,
  created_at timestamptz not null default now(),
  primary key (case_id, user_id)
...
create index if not exists safety_alert_acks_user_idx
...
drop policy if exists "safety_alert_acks_select_own" on public.safety_alert_acks;
...
drop policy if exists "safety_alert_acks_insert_own" on public.safety_alert_acks;
create policy "safety_alert_acks_insert_own"
  on public.safety_alert_acks for insert
  with check (auth.uid() = user_id);

drop policy if exists "safety_alert_acks_delete_own" on public.safety_alert_acks;
...
-- ============================================================================
-- 0036_push_notifications.sql — real (Web Push) push notifications
-- ============================================================================
-- push_subscriptions is a new table, so it needs its own env vars set on the
-- deployment for pushes to actually send: VAPID_PUBLIC_KEY, VAPID_PRIVATE_KEY,
-- VAPID_SUBJECT, NEXT_PUBLIC_VAPID_PUBLIC_KEY (see .env.example). Without
-- them the app still works — sendPushToUsers() no-ops and in-app
...
-- The four existing fan_out_* functions change return type (void -> setof
-- uuid), which `create or replace` cannot do — each is dropped and recreated
-- with its guard/dedupe logic otherwise unchanged.
...
from (
  values
    ...
    ('fan_out_case_update returns setof uuid (push dispatch)',
     coalesce((select pg_get_function_result(oid) = 'SETOF uuid'
               from pg_proc where proname = 'fan_out_case_update'), false)),
    ('function: notify_new_message',
     to_regprocedure('public.notify_new_message(uuid)') is not null)
